    ...
    <div ref={containerRef} id="hero" className="relative h-[400vh] bg-[#000000] text-[#FFFFFF]">
      ...
      <div className="sticky top-0 w-full h-screen overflow-hidden flex flex-col justify-between items-center bg-[#000000]">
        ...
        <motion.div
          ...
          className="relative z-20 w-full h-full max-w-[1400px] mx-auto px-6 flex flex-col justify-between items-center pt-24 pb-8"
        >
          {/* PERFECTLY CENTERED TRANSFORM-ORIGIN CENTER TYPOGRAPHY GROWTH (48px -> 60px -> 90px -> 140px -> 220px -> 320px / 90vw) */}
          <div className="text-center pt-4 sm:pt-6 z-30 flex flex-col items-center justify-center w-full">
            <motion.div
              style={{ scale: nameScale, opacity: nameOpacity }}
              className="origin-center text-center flex items-center justify-center w-full"
            >
              <h1 className="font-['Bebas_Neue',sans-serif] font-bold text-[48px] text-[#FFFFFF] tracking-[0.25em] uppercase text-center leading-none drop-shadow-lg whitespace-nowrap max-w-[90vw] mx-auto">
                GRACE RESHAL LEWIS
              </h1>
            </motion.div>

            {/* PROFESSIONAL TITLE (Inter 16px 0.3em 70% opacity) */}
            <motion.p
              style={{ opacity: overlayOpacity }}
              className="mt-4 font-['Inter',sans-serif] text-[16px] font-normal tracking-[0.3em] text-[#BFBFBF]/70 uppercase text-center flex items-center justify-center gap-2 flex-wrap"
            >
          ...
          {/* CENTERED TRANSPARENT PNG PHOTO (Slower movement y: 0 to y: -250px max) */}
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 pt-16">
            <motion.div style={{ y: imageY }} className="relative flex items-center justify-center">
              <img
                src={graceImg}
                alt="Grace Reshal Lewis"
                className="w-[300px] sm:w-[420px] lg:w-[520px] h-auto object-contain filter contrast-[1.06] brightness-[0.96] drop-shadow-[0_20px_50px_rgba(0,0,0,0.9)]"
              />
            </motion.div>
          </div>
          ...
          <motion.div
            style={{ opacity: overlayOpacity }}
            className="w-full flex flex-col items-center gap-6 z-30 pt-auto mt-auto pointer-events-auto"
          >
            {/* Action Buttons */}
            <div className="flex flex-wrap items-center justify-center gap-4">
              <a
                href="#projects"
                className="group relative inline-flex items-center gap-2.5 px-8 py-3.5 rounded-full border border-[#FFFFFF] bg-transparent text-[#FFFFFF] font-['Inter',sans-serif] text-xs font-semibold tracking-[0.2em] uppercase transition-all duration-300 hover:bg-[#FFFFFF] hover:text-[#000000] hover:scale-[1.03]"
              >
              ...
              <a
                ...
                download
                className="inline-flex items-center gap-2 px-8 py-3.5 rounded-full border border-[#FFFFFF]/40 bg-transparent text-[#FFFFFF] font-['Inter',sans-serif] text-xs font-semibold tracking-[0.2em] uppercase transition-all duration-300 hover:bg-[#FFFFFF] hover:text-[#000000] hover:scale-[1.03]"
              >
                <Download className="w-4 h-4 text-[#BFBFBF]" />
            ...
            <div className="flex items-center gap-3">
              <a
                ...
                rel="noreferrer"
                aria-label="Github"
                className="w-10 h-10 rounded-full border border-[#FFFFFF]/30 bg-transparent flex items-center justify-center text-[#BFBFBF] hover:bg-[#FFFFFF] hover:text-[#000000] hover:border-[#FFFFFF] transition-all duration-300"
              >
                <Github size={16} />
              </a>
              <a
            ...
            {/* Scroll Indicator */}
            <div className="flex flex-col items-center gap-1.5 text-[#BFBFBF] pt-2 pointer-events-none">
              <div className="w-5 h-8 rounded-full border-2 border-[#FFFFFF]/40 flex items-start justify-center p-1">
                <div className="w-1 h-2 bg-[#FFFFFF] rounded-full animate-bounce" />
              </div>
              <span className="font-['Inter',sans-serif] text-[10px] tracking-[0.3em] uppercase text-[#BFBFBF]">
                SCROLL TO EXPLORE
              </span>
            </div>
          </motion.div>
